...
async def callback_processing(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    # обработка города по геолокации
        ...
        # double_side = mybd.select_info(option_by_printers from printers where ID equals printer_id), вытаскиваем значение возможности
        # двойной печати, для того, чтобы вывести необходимую клавиатуру
        async with state.proxy() as data:
        ...
        # По идее тут все параметры для расчета цены имеются, остается вытащить только количество страниц, и вставить в ответ
        # ну и расчет какой-то сделать, вроде параметры все для этого есть
        await bot.send_message(callback_query.from_user.id, f"Ваш заказ:\n\n"
                                                            f"Количество страниц: {тут количество страниц}\n"
        ...
        await bot.send_invoice(
            ...
            provider_token=PAYMENTS_PROVIDER_TOKEN,#токен надо в гит игнор засунуть, хз как, вот он: PAYMENTS_PROVIDER_TOKEN = "381764678:TEST:28424"
            currency='rub',
